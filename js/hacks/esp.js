...
app.controller('hackESP', ['$scope', function($scope) {
  ...
  setInterval(function(){
    ...
    for(var i = 0; i < objectCount; i++){
      ...
      if(isWeapon(classid) && !entityDormant) drawGlowPlayer(i, "item");
      ...
      if($scope.ESP.target == 'ENEMIES' && isEnemy){
        drawGlowPlayer(cPlayerGlowIndex, 'enemy');
      } else if ($scope.ESP.target == 'FRIENDLIES' && !isEnemy){
        drawGlowPlayer(cPlayerGlowIndex, 'team');
      } else if($scope.ESP.target == 'BOTH'){
        drawGlowPlayer(cPlayerGlowIndex, isEnemy ? 'enemy' : 'team');
      }
    }
  ...
  function isWeapon(id) {
    return [1, 8, 8, 9, 13, 29, 31, 39, 40, 41, 64, 82, 83, 84, 89, 90, 93, 94, 104,
       115, 122, 126, 127, 176, 197, 198, 199, 200, 201, 203, 204, 205, 206, 207,
       208, 209, 210, 211, 212, 213, 214, 215, 216, 217, 218, 219, 220, 221, 222,
       223, 224, 225, 226, 227, 228, 229, 230, 231, 232, 233, 234, 235, 236, 240].indexOf(id) != -1;
  }

  /* Writes to memory to make a player glow */
  function drawGlowPlayer(glowIndex, side){
    /* hex goes up by the size of the data type (e.g. float is 4, boolean is 1),
  	...
  	memoryjs.writeMemory(glowPointer + ((glowIndex * 0x38) + 0x4), ($scope.ESP.colours[side].r / 255) * 1.4, 'float');
  	memoryjs.writeMemory(glowPointer + ((glowIndex * 0x38) + 0x8), ($scope.ESP.colours[side].g / 255) * 1.4, 'float');
  	memoryjs.writeMemory(glowPointer + ((glowIndex * 0x38) + 0xC), ($scope.ESP.colours[side].b / 255) * 1.4, 'float');
  	memoryjs.writeMemory(glowPointer + ((glowIndex * 0x38) + 0x10), ($scope.ESP.colours[side].a / 100) * 1.4, 'float');
